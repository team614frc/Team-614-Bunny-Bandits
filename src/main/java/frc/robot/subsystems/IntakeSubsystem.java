...
public IntakeSubsystem() {
    ...
    intakeMotor.setSmartCurrentLimit(IntakeConstants.MOTOR_CURRENT_LIMIT); //makes sure the motors don't take too much amps from the battery
    intakeMotor.setInverted(false); //Allows the motors to be able to spin in different directions. 
    intakeMotor.setIdleMode(CANSparkFlex.IdleMode.kCoast); //Causes the motors to slow down in motion when no action is being commanded.
